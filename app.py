...
def filter_already_processed_payouts(df_payout):
    ...
    # Trennen in neu und bereits vorhanden
...
if uploaded_orders_db:
    try:
        if uploaded_orders_db.name.endswith(('.xlsx', '.xls')):
            ...
            df_ref = pd.read_excel(uploaded_orders_db, sheet_name=xls.sheet_names[0])
            if 'Bestellnummer' not in df_ref.columns and 'Column2' in df_ref.columns:
                df_ref = pd.read_excel(uploaded_orders_db, sheet_name=xls.sheet_names[0], header=1)
        else:
            df_ref = pd.read_csv(uploaded_orders_db, sep=None, engine='python')

        ref_order_col = next((c for c in df_ref.columns if 'bestell' in str(c).lower()), None)
        ref_title_col = next((c for c in df_ref.columns if any(x in str(c).lower() for x in ['angebotstitel', 'artikelbezeichnung', 'titel', 'bezeichnung']) and 'nummer' not in str(c).lower() and 'id' not in str(c).lower()), None)
        ref_sku_col = next((c for c in df_ref.columns if 'bestandseinheit' in str(c).lower() or 'sku' in str(c).lower()), None)

        if ref_order_col and ref_title_col:
            df_ref['Match_Key'] = df_ref[ref_order_col].apply(lambda x: re.sub(r'[^A-Za-z0-9]', '', str(x).strip().replace('.0','')).upper().lstrip('0'))
            df_ref['Title_Val'] = df_ref[ref_title_col]
            df_ref['SKU_Val'] = df_ref[ref_sku_col] if ref_sku_col else ''
            
            added = save_orders_to_db(df_ref)
            st.sidebar.success(f"✅ {added} Einträge in DB gespeichert/aktualisiert!")
        else:
            st.sidebar.error("❌ Spalten im Bestellbericht nicht erkannt.")
...
with st.sidebar.expander(f"📦 DB-Status ({db_orders_count} Artikel | {db_payouts_count} Payouts)"):
    st.write(f"• Saved Titles: **{db_orders_count}**")
    st.write(f"• Processed Payouts: **{db_payouts_count}**")
...
if uploaded_payout:
    try:
        ...
        # Duplikate-Prüfung gegenüber früheren Payouts aus der DB
        df_payout, skipped_count = filter_already_processed_payouts(df_payout)
        if skipped_count > 0:
            st.warning(f"ℹ️ **{skipped_count} Transaktion(en)** wurden übersprungen, da sie bereits in einer früheren Sitzung hochgeladen wurden.")
        ...
        if df_payout.empty:
            ...
            # Artikelnamen aus der lokalen SQLite-Datenbank laden
            ...
            if not df_grp_b.empty:
                if st.button("🚀 JETZT GRUPPE B AUTOMATISCH IN LEXOFFICE ANLEGEN", type="primary"):
                    ...
                    if not contact_id:
                        ...
                        if inv_id:
                            # Payouts in der Historie speichern, damit sie nicht doppelt hochgeladen werden
