...
def initialize(db, directory):
    ...
    if supabase_store.enabled():
        saved, _ = supabase_store.get_json('state/partner_invoices.json', default={'invoices': [], 'positions': []})
        path = None
    else:
...
def mirror(db, directory):
    ...
    import supabase_store
    if supabase_store.enabled():
        current, version = supabase_store.get_json('state/partner_invoices.json', default={'invoices': [], 'positions': []})
        if current != value:
            supabase_store.put_json('state/partner_invoices.json', value, version)
        return
    path = directory/'Settlement_Partner_Invoices.json'
